...
- 🎯 Entry points to execute your program using `python -m <service_a>` or `$ service_a` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/CJSmith-0141/service_A/branch/main/graph/badge.svg?token=service_A_token_here)](https://codecov.io/gh/CJSmith-0141/service_A)
[![CI](https://github.com/CJSmith-0141/service_A/actions/workflows/main.yml/badge.svg)](https://github.com/CJSmith-0141/service_A/actions/workflows/main.yml)

Awesome service_a created by CJSmith-0141

## Install

from source
```bash
git clone https://github.com/CJSmith-0141/service_A service_a
cd service_a
make install
```
...
```bash
pip install service_a
```

## Executing

```bash
$ service_a run --port 8080
```

or

```bash
python -m service_a run --port 8080
```

or

```bash
$ uvicorn service_a:app
```
...
```bash
❯ service_a --help
Usage: service_a [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ service_a create-user --help
Usage: service_a create-user [OPTIONS] USERNAME PASSWORD
...
```bash
service_a create-user admin admin --superuser
```
...
Run with `service_a run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `service_a create-user`.
...
-----------------------------------------------------
service_a/__init__.py              4      0   100%
service_a/app.py                  16      1    94%
service_a/cli.py                  21      0   100%
service_a/config.py                5      0   100%
service_a/db.py                   10      0   100%
service_a/models/__init__.py       0      0   100%
service_a/models/content.py       47      1    98%
service_a/routes/__init__.py      11      0   100%
service_a/routes/content.py       52     25    52%
service_a/routes/security.py      15      1    93%
service_a/routes/user.py          52     26    50%
service_a/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in service_a/default.toml.
